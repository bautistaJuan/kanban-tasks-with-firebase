...
const useApp = () => {
  ...
  const updateBoardData = async (boardId, tabs) => {
    ...
    try {
      ...
      setToastr("Error al actualizar el tablero");
      throw err;
    }
  };
  ...
  const fetchBoard = async boardId => {
    ...
    try {
      ...
      setToastr("Error traer el tablero");
      throw err;
    }
  };
  ...
  const createBoard = async ({ name, color }) => {
    ...
    try {
      ...
      setToastr("Error creando el tablero");
      throw err;
    }
  };
  ...
  const fetchBoards = async setLoading => {
    try {
      ...
      const boards = querySnapshot.docs.map(doc => ({
        ...doc.data(),
        id: doc.id,
        createdAt: doc.data().createdAt.toDate().toLocaleString("es-AR"),
      }));

      setBoards(boards);
    } catch (err) {
      setToastr("Error al traer los tableros");
    } finally {
